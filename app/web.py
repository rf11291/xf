...
def create_app() -> Flask:
    ...
    def dashboard():
        ...
        total_subs = len(db.list_all_subscription_details(limit=5000))
        expiring_soon = db.list_subscriptions_expiring_within(30, limit=10)
        today = dt.date.today()
    ...
    def customers():
        ...
        items = db.list_customers(offset=0, limit=200)
        if q:
            items = [
                c
                for c in items
                if q in (c.get("email") or "").lower() or q in (c.get("name") or "").lower()
            ]
    ...
    def products():
        q = request.args.get("q", "").strip().lower()
        items = db.list_products(offset=0, limit=200)
        if q:
    ...
    def subscriptions():
        q = request.args.get("q", "").strip().lower()
        items = db.list_all_subscription_details(limit=5000)
        if q:
            items = [
                s
                for s in items
                if q in (s.get("customer_email") or "").lower()
        ...
        for item in items:
            exp = dt.date.fromisoformat(str(item["expires_at"]))
            item["days_left"] = (exp - today).days
        return render_template("subscriptions.html", items=items, q=q)
    ...
    @login_required
    def logs():
        items = db.list_reminder_daily_logs(offset=0, limit=200)
